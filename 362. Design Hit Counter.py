# UberYandexDatabricksDesign a hit counter which counts the number of hits received in the past 5 minutes
...
class HitCounter:
    ...
    def __init__(self):
        self.tms = defaultdict(int)

    def hit(self, timestamp: int) -> None:
        self.tms[timestamp] += 1

    def getHits(self, timestamp: int) -> int:
        return sum([v for i, v in self.tms.items() if (timestamp - 300) < i <= timestamp])
