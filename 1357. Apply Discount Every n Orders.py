...
# When a customer is paying, their bill is represented as two parallel integer arrays product and amount, where the jth product they purchased has an ID of product[j], 
# and amount[j] is how much of the product they bought. Their subtotal is calculated as the sum of each amount[j] * (price of the jth product).
...
class Cashier:
    ...
    def getBill(self, product: List[int], amount: List[int]) -> float:
        self.cnt += 1
        res = 0
        for product_id, amt in zip(product, amount):
            res += (self.products.get(product_id) * amt)
        return res * self.discount if self.cnt % self.n == 0 else res
